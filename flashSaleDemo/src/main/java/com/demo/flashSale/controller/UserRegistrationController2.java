...
public class UserRegistrationController2 {
	...
	private CyclicBarrier _cyclicBarrier = null;
	...
	public String regSuccess(@PathVariable("emailId") String emailId, @PathVariable("regId") String regId,
			Model model) {
		...
		if (entity.getStatus().equalsIgnoreCase("ACT")) {
			...
			model.addAttribute("buyBtnDisable", "true");
			...
			if (new Date().compareTo(dateEntity.getDateTime()) < 0) {
				...
				model.addAttribute("buyBtnDisable", "false");
				...
				model.addAttribute("stockAvailable",
						_cyclicBarrier.getParties() - _cyclicBarrier.getNumberWaiting());
				model.addAttribute("totalStock", _cyclicBarrier.getParties());
